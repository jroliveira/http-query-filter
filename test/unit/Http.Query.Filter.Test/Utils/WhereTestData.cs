namespace Http.Query.Filter.Test.Utils
{
    ...
    {
        protected static readonly Func<string, string, Comparison, Condition> Item = (field, value, comparison) => new Condition(field, value, comparison);
        protected static readonly Func<string, string, Comparison, Where> Field = (field, value, comparison) => Fields(data => data.Add(Item(field, value, comparison)));
